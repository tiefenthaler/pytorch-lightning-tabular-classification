...
from src.tabular_lightning.encoders import OrdinalEncoderExtensionUnknowns


@pytest.fixture
def setup_transformer() -> OrdinalEncoderExtensionUnknowns:
    """Fixture to set up the custom transformer."""
    return OrdinalEncoderExtensionUnknowns()


def test_transform_numpy_array(setup_transformer) -> None:
...
def test_transform_series(setup_transformer) -> None:
...
def test_transform_dataframe(setup_transformer) -> None:
    """Test transformation with a pandas DataFrame."""
    input_df = pd.DataFrame({"col1": [1, 2, -1], "col2": [-1, 3, 4]})
    expected_output_df = pd.DataFrame({"col1": [2, 3, 0], "col2": [0, 4, 5]})

    transformed_df = setup_transformer.transform(input_df)
    pd.testing.assert_frame_equal(transformed_df, expected_output_df)


def test_inverse_transform_numpy_array(setup_transformer) -> None:
...
def test_inverse_transform_series(setup_transformer) -> None:
...
def test_inverse_transform_dataframe(setup_transformer) -> None:
    """Test inverse transformation with a pandas DataFrame."""
    input_df = pd.DataFrame({"col1": [2, 3, 0], "col2": [0, 4, 5]})
    expected_output_df = pd.DataFrame({"col1": [1, 2, -1], "col2": [-1, 3, 4]})

    inverse_df = setup_transformer.inverse_transform(input_df)
    pd.testing.assert_frame_equal(inverse_df, expected_output_df)


def test_transform_invalid_input(setup_transformer) -> None:
    """Test transformation with invalid input type."""
    with pytest.raises(
        ValueError, match="Input must be a pandas DataFrame, Series, or NumPy array"
    ):
